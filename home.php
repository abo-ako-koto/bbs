...
// メッセージを丸ごと収納して配列化
$message_array = array();
// 空白だった時のエラーメッセージ（2種類なのでarray）
$error_message = array();
// サニタイズ（不正コード対策）したメッセージ
$clean = array();
...
// データベースに接続
// 定数への変更なのでクォーテーションいらない
$mysqli = new mysqli( DB_HOST, DB_USER, DB_PASS, DB_NAME);
// 接続エラーの確認
if( $mysqli->connect_errno ) {
    $error_message[] = 'データの読み込みに失敗しました。 エラー番号 '.$mysqli->connect_errno.' : '.$mysqli->connect_error;
} else {
    /* 1つ目の「取得するカラム名」には、「id」以外の３つのカラムを指定し、データを取得するテーブルは「message」を指定します。
...
*/
    ...
    if( $res ) {
        // データをファイル読み込みのときと同様の配列形式で取得したいため、メソッドに「MYSQLI_ASSOC」を指定
